/**
 * LeviTracker v10 — learned from libAxioClient.so
 *
 * What AxioClient taught us:
 * 1. GlossHook for eglSwapBuffers — same as us, confirmed correct
 * 2. Touch: uses GetPreloaderInput (exact symbol) from libpreloader.so
 *    NOT PreloaderInput, NOT RegisterPreloaderInput — it's GetPreloaderInput
 * 3. Coords: AxioClient only shows YOUR coords by reading player object
 *    directly at offset 0x1d4(X) 0x1d8(Y) 0x1dc(Z) — version-specific
 * 4. For ALL players: packet scan (recvfrom/sendto) is the right approach
 *    since it's version-independent
 * 5. mprotect for libc hooks works — AxioClient calls mprotect directly
 * 6. GlossInit is called with NO args (confirmed from NeuralTheft pattern)
 *
 * Key fix for "mod not loading": the mod WAS loading but logcat filter
 * was wrong. Our tag "LT" is too short — Levi filters it. Use longer tag.
 */
...
// ── GetPreloaderInput (exact name from AxioClient strings) ────
// Returns pointer to PreloaderInput object which has a touch callback
typedef void* (*FnGetPreloaderInput)();
static FnGetPreloaderInput pGetPreloaderInput = nullptr;

// The PreloaderInput object has a virtual method to register callback
// From AxioClient: "Successfully registered with PreloaderInput"
// Signature from common Levi modding: registerCallback(void* cb)
typedef void (*FnRegisterCallback)(void* preloaderInput, void* callback);
// We'll try vtable slot 0 and 1

// ── ARM64 trampoline hook for libc functions ──────────────────
struct Hook { void* orig; bool ok; };
static Hook makeHook(void* target, void* repl) {
    Hook h={nullptr,false};
    if(!target||!repl||(uintptr_t)target&3) return h;
    void* tr=mmap(nullptr,0x1000,PROT_READ|PROT_WRITE|PROT_EXEC,
                  MAP_PRIVATE|MAP_ANONYMOUS,-1,0);
    if(tr==MAP_FAILED) return h;
    memcpy(tr,target,16);
    uint32_t* j=(uint32_t*)((uint8_t*)tr+16);
    j[0]=0x58000051; j[1]=0xD61F0220;
    uint64_t bk=(uint64_t)target+16; memcpy(&j[2],&bk,8);
    __builtin___clear_cache((char*)tr,(char*)tr+48);
    h.orig=tr;
    uintptr_t pg=(uintptr_t)target&~0xFFFull;
    // Use direct syscall — AxioClient uses mprotect@LIBC which works fine
    if(mprotect((void*)pg,0x2000,PROT_READ|PROT_WRITE|PROT_EXEC)!=0) {
        munmap(tr,0x1000); return h;
    }
    uint32_t* c=(uint32_t*)target;
    c[0]=0x58000051; c[1]=0xD61F0220;
    uint64_t dst=(uint64_t)repl; memcpy(&c[2],&dst,8);
...
static void pAd(const uint8_t*d,size_t l,size_t p){
    ...
    pthread_mutex_unlock(&gLk);
    LOG("AddPlayer: %s rid=%lld",nm,(long long)rid);
}
...
static void pAI(const uint8_t*d,size_t l,size_t p){
    // PlayerAuthInput 0x90 — your exact position every tick
...
// ── Touch via /dev/input (reliable, no PreloaderInput needed) ─
...
static float           gTMaxX=1080.f,gTMaxY=2400.f;
...
static void* touchThread(void*){
    ...
    if(dir){struct dirent*e;
        while((e=readdir(dir))){
            ...
            if(ioctl(fd,EVIOCGBIT(EV_ABS,sizeof(bits)),bits)>=0)
                if(bits[6]&(1<<(ABS_MT_POSITION_X-48))){
                    strncpy(path,p,63);close(fd);break;
                }
            close(fd);
        }
        closedir(dir);
    }
    LOG("touch device: %s",path[0]?path:"not found");
    ...
    if(ax.maximum>0)gTMaxX=(float)ax.maximum;
    if(ay.maximum>0)gTMaxY=(float)ay.maximum;
    LOG("touch range %.0fx%.0f",gTMaxX,gTMaxY);
...
// Same shader as AxioClient (confirmed from binary)
...
static bool glInit(){
    auto mkSh=[](GLenum t,const char*s)->GLuint{
        ...
        if(!ok){char b[512];glGetShaderInfoLog(h,512,0,b);LOGE("sh:%s",b);}
        return h;
    };
    ...
    if(!ok){char b[512];glGetProgramInfoLog(gProg,512,0,b);LOGE("lnk:%s",b);return false;}
    ...
    for(int g=0;g<64;g++)for(int c=0;c<GW;c++)for(int r=0;r<GH;r++){
        bool lit=(kF[g][c]&(1<<r))!=0;int i=(r*AW+g*GW+c)*4;
    ...
    free(px);gGlOk=true;LOG("GL ok prog=%u tex=%u",gProg,gTex);return true;
}
...
static void processTouch(){
    ...
    if(!t.fresh)return;
    float sx=t.x/gTMaxX*gSW, sy=t.y/gTMaxY*gSH;
    ...
    if(t.down&&!gWasDown){
        ...
        if(fabsf(dx)>5||fabsf(dy)>5){
            gBtnOffX=gBtnDragBX-dx/gTMaxX*gSW;gBtnOffY=gBtnDragBY+dy/gTMaxY*gSH;
...
static ssize_t hkSend(int fd,const void*buf,size_t l,int f,const struct sockaddr*a,socklen_t al){
    ssize_t n=oSend(fd,buf,l,f,a,al);
...
static void* initThread(void*){
    ...
    // Load libpreloader.so — same as AxioClient
    void*pre=dlopen("libpreloader.so",RTLD_NOW|RTLD_NOLOAD);
    if(!pre)pre=dlopen("libpreloader.so",RTLD_NOW|RTLD_GLOBAL);
    if(pre){
        // GlossHook only — never GlossSymbol/GlossOpen (crashes on Android 16)
        pGlossHook=(FnGlossHook)dlsym(pre,"GlossHook");
        pGlossInit=(FnGlossInit)dlsym(pre,"GlossInit");
        // GetPreloaderInput — exact symbol name from AxioClient strings
        pGetPreloaderInput=(FnGetPreloaderInput)dlsym(pre,"GetPreloaderInput");
        LOG("GlossHook=%p GlossInit=%p GetPreloaderInput=%p",
            pGlossHook,pGlossInit,pGetPreloaderInput);
    ...
    if(!mc){LOGE("MC never loaded");return nullptr;}
    LOG("MC loaded");sleep(2);
    ...
    LOG("eglSwapBuffers=%p",swapSym);
    if(pGlossHook&&swapSym){
        int r=pGlossHook(swapSym,(void*)hkSwap,(void**)&oSwap);
        LOG("hookSwap r=%d oSwap=%p",r,oSwap);
    }

    // Hook recvfrom + sendto via mprotect trampoline (NOT GlossHook — crashes on libc)
    void*libc=dlopen("libc.so",RTLD_NOW|RTLD_NOLOAD);
    if(!libc)libc=dlopen("libc.so",RTLD_NOW);
    if(libc){
        void*rv=dlsym(libc,"recvfrom");
        if(rv){Hook h=makeHook(rv,(void*)hkRecv);if(h.ok){oRecv=(FnRecv)h.orig;LOG("recv ok");}
               else LOGE("recv hook failed");}
        void*sv=dlsym(libc,"sendto");
        if(sv){Hook h=makeHook(sv,(void*)hkSend);if(h.ok){oSend=(FnSend)h.orig;LOG("send ok");}
               else LOGE("send hook failed");}
    }

    // Start touch reader
    pthread_t tt;pthread_create(&tt,nullptr,touchThread,nullptr);pthread_detach(tt);

    LOG("LeviTracker v10 ACTIVE — swap=%p recv=%p send=%p",oSwap,oRecv,oSend);
    return nullptr;
}
...
static void onLoad(){
    LOG("LeviTracker v10 loading on pid=%d",(int)getpid());
