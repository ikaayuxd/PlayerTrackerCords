/**
 * LeviTracker v13 — STABLE BASE
 *
 * REMOVED: All GOT/ELF parsing (kept crashing with bad addresses)
 * REMOVED: recvfrom/sendto hooks (can't safely hook libc OR MC on this device)
 *
 * WHAT WORKS (proven in previous runs without crash):
 *   1. GlossHook -> eglSwapBuffers  -> draw overlay
 *   2. /dev/input touch thread      -> tap FAB, toggle menu
 *   3. YOUR coords via PlayerAuthInput in sendto
 *      BUT we hook sendto via a different safe method:
 *      We use GlossHook on sendto's address from libminecraftpe.so's own
 *      import — found by scanning MC's PLT stubs (the actual executable
 *      thunk bytes at mcBase + known PLT offset), NOT by ELF parsing.
 *
 * FOR OTHER PLAYERS:
 *   We rely on MovePlayer packets from recvfrom.
 *   Since we can't safely hook recvfrom, we get other player coords
 *   by reading MC memory directly — same method as AxioClient.
 *   We find the EntityRegistry via a known chain from ClientInstance.
 *   This is done safely in the swap hook (read-only, no patching).
 *
 * FALLBACK if memory read fails: show "JOIN A SERVER" for others.
 * YOUR coords still show from PlayerAuthInput via sendto GlossHook.
 */
...
static int phdrCb(struct dl_phdr_info*i,size_t,void*d){
    if(i->dlpi_name&&strstr(i->dlpi_name,"libminecraftpe.so")){
        *(uintptr_t*)d=(uintptr_t)i->dlpi_addr;return 1;}
    return 0;
}
...
static void pAI(const uint8_t*d,size_t l,size_t p){
    // PlayerAuthInput 0x90 — our own exact position every tick
...
// ── sendto hook via GlossHook on libc symbol ──────────────────
// GlossHook can patch EGL (works), so let's try sendto from libc
// via dlsym — GlossHook handles the patching safely internally.
...
static EGLBoolean hkSwap(EGLDisplay d,EGLSurface s){
    ...
    gSwapCount++;
    if(gSwapCount==1||gSwapCount==60)
        LOG("hkSwap firing! frame=%d size=%.0fx%.0f",gSwapCount,gSW,gSH);
    draw();
...
static void*initThread(void*){
    ...
    // Hook eglSwapBuffers — GlossHook first, ARM64 trampoline fallback
    ...
    LOG("swapSym=%p GlossHook=%p",swapSym,pGlossHook);
    if(pGlossHook&&swapSym){
        int r=pGlossHook(swapSym,(void*)hkSwap,(void**)&oSwap);
        LOG("GlossHook swap r=%d oSwap=%p",r,oSwap);
    }
    // Fallback: ARM64 trampoline directly on libEGL.so (always patchable)
    if(!oSwap&&swapSym){
        LOG("Trying ARM64 trampoline on EGL...");
        void*tr=mmap(nullptr,0x1000,PROT_READ|PROT_WRITE|PROT_EXEC,
                     MAP_PRIVATE|MAP_ANONYMOUS,-1,0);
        if(tr!=MAP_FAILED){
            memcpy(tr,swapSym,16);
            uint32_t*j=(uint32_t*)((uint8_t*)tr+16);
            j[0]=0x58000051;j[1]=0xD61F0220;
            uint64_t bk=(uint64_t)swapSym+16;memcpy(&j[2],&bk,8);
            __builtin___clear_cache((char*)tr,(char*)tr+48);
            oSwap=(FnSwap)tr;
            uintptr_t pg=(uintptr_t)swapSym&~0xFFFull;
            if(mprotect((void*)pg,0x2000,PROT_READ|PROT_WRITE|PROT_EXEC)==0){
                uint32_t*c=(uint32_t*)swapSym;
                c[0]=0x58000051;c[1]=0xD61F0220;
                uint64_t dst=(uint64_t)hkSwap;memcpy(&c[2],&dst,8);
                __builtin___clear_cache((char*)swapSym,(char*)swapSym+16);
                mprotect((void*)pg,0x2000,PROT_READ|PROT_EXEC);
                LOG("ARM64 trampoline OK oSwap=%p",oSwap);
            } else {
                LOGE("mprotect EGL failed");
                oSwap=nullptr;munmap(tr,0x1000);
            }
        }
    }
    LOG("final oSwap=%p",oSwap);

    // Hook sendto via GlossHook — same mechanism as eglSwapBuffers
    // GlossHook handles the patching, we just give it the address
    void*libc=dlopen("libc.so",RTLD_NOW|RTLD_NOLOAD);
    if(!libc)libc=dlopen("libc.so",RTLD_NOW);
    if(libc&&pGlossHook){
        void*sv=dlsym(libc,"sendto");
        if(sv){
            int r=pGlossHook(sv,(void*)hkSend,(void**)&oSend);
            LOG("sendto r=%d oSend=%p",r,oSend);
        }
        void*rv=dlsym(libc,"recvfrom");
        if(rv){
            int r=pGlossHook(rv,(void*)hkRecv,(void**)&oRecv);
            LOG("recvfrom r=%d oRecv=%p",r,oRecv);
        }
    }
...
static void onLoad(){
    LOG("LeviTracker v13 pid=%d",(int)getpid());
    const char*fp[]={
