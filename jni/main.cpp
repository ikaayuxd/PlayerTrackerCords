 ...
 *   Hooking recvfrom/sendto in libc.so crashes on Android 16.
 *   libc lives in /apex/ — SELinux marks it non-writable even with mprotect.
 *   GlossHook also crashes on it (proved by crash log v9).
 *
 * SOLUTION:
 *   Don't hook libc recvfrom/sendto at all.
 *   Instead: find recvfrom/sendto as they appear in libminecraftpe.so's PLT
 *   and hook THOSE entries — they live in patchable MC memory.
 *   MC's PLT entries for recvfrom/sendto are just 8-byte stubs we can patch.
 *
 *   Alternatively (simpler & proven): hook only eglSwapBuffers via GlossHook,
 *   and get coords by reading MC memory directly (like AxioClient does).
 *   We use dl_iterate_phdr to find MC base, then read LocalPlayer at known offsets.
 *   For OTHER players: hook recvfrom inside MC's own socket calls via PLT patch.
 *
 * FINAL SAFE ARCHITECTURE:
 *   1. GlossHook -> eglSwapBuffers (overlay render) [PROVEN SAFE]
 *   2. dl_iterate_phdr -> find MC base -> read LocalPlayer XYZ [like AxioClient]
 *   3. MC PLT hook -> recvfrom -> scan incoming packets for other players
 *   4. Touch via /dev/input thread
 *   5. NO mprotect on libc, NO GlossHook on libc
 */
...
// ── MC base address (found via dl_iterate_phdr) ───────────────
static uintptr_t gMcBase = 0;
static size_t    gMcSize = 0;

struct PhdrCbData { const char* name; uintptr_t base; size_t size; };
static int phdrCb(struct dl_phdr_info* info, size_t, void* data) {
    if (!info->dlpi_name) return 0;
    auto* d = (PhdrCbData*)data;
    if (strstr(info->dlpi_name, d->name)) {
        d->base = (uintptr_t)info->dlpi_addr;
        // Find largest PT_LOAD segment size
        for (int i = 0; i < info->dlpi_phnum; i++) {
            if (info->dlpi_phdr[i].p_type == PT_LOAD) {
                size_t end = info->dlpi_phdr[i].p_vaddr + info->dlpi_phdr[i].p_memsz;
                if (end > d->size) d->size = end;
            }
        }
        return 1;
    }
...
// ── LocalPlayer XYZ offsets (from AxioClient reverse) ─────────
// AxioClient reads: player_ptr + 0x1d4 = X, +0x1d8 = Y, +0x1dc = Z
// These are relative to the Actor/Player object base
// We need to find LocalPlayer pointer from MC's global state
//
// Standard Bedrock pointer chain (version-independent via symbol):
//   ClientInstance* -> LocalPlayer* -> Vec3 position
// We find ClientInstance via GlossSymbol on its known export OR
// scan MC's GOT for a known function and walk back.
//
// SIMPLER: we hook eglSwapBuffers which MC calls every frame.
// When our hook runs, MC's TLS has the current thread context.
// We can walk the stack to find player pointers.
//
// SIMPLEST & MOST RELIABLE: just scan packets. The issue was HOW we hook.
// Instead of hooking libc recvfrom, hook MC's IMPORT of recvfrom.
// MC's PLT entry for recvfrom is inside libminecraftpe.so which IS patchable.

static Hook* g_mcRecvHook = nullptr; // reuse our Hook struct

struct Hook2 { void* orig; bool ok; };
static Hook2 patchGOT(uintptr_t mcBase, size_t mcSize, const char* symName, void* replacement) {
    Hook2 h = {nullptr, false};
    if (!mcBase) return h;

    // Parse ELF of MC to find .got.plt and patch the entry for symName
    // The ELF header is at mcBase
    auto* ehdr = (Elf64_Ehdr*)mcBase;
    if (ehdr->e_ident[0] != 0x7f || ehdr->e_ident[1] != 'E') return h;
    ...
    // Walk dynamic section to find SYMTAB, STRTAB, JMPREL, PLTRELSZ
    ...
    for (Elf64_Dyn* d = dyn; d->d_tag != DT_NULL; d++) {
        switch(d->d_tag) {
            case DT_SYMTAB:  symtab  = (Elf64_Sym*) (mcBase + d->d_un.d_ptr - mcBase < mcSize ? d->d_un.d_ptr : mcBase + (d->d_un.d_ptr & 0xFFFFFF)); break;
            case DT_STRTAB:  strtab  = (const char*)(mcBase + d->d_un.d_ptr - mcBase < mcSize ? d->d_un.d_ptr : mcBase + (d->d_un.d_ptr & 0xFFFFFF)); break;
            case DT_JMPREL:  jmprel  = (Elf64_Rela*)(mcBase + d->d_un.d_ptr - mcBase < mcSize ? d->d_un.d_ptr : mcBase + (d->d_un.d_ptr & 0xFFFFFF)); break;
            case DT_PLTRELSZ:pltsz   = d->d_un.d_val; break;
        }
    }
    if (!symtab || !strtab || !jmprel || !pltsz) return h;
    ...
    for (size_t i = 0; i < nrel; i++) {
        ...
        if (strcmp(name, symName) == 0) {
            // Found it — patch the GOT entry
            uintptr_t* gotEntry = (uintptr_t*)(mcBase + jmprel[i].r_offset);
            LOG("GOT entry for %s at %p, current val=%p", symName, gotEntry, (void*)*gotEntry);

            uintptr_t pg = (uintptr_t)gotEntry & ~0xFFFull;
            if (mprotect((void*)pg, 0x1000, PROT_READ|PROT_WRITE) != 0) {
                LOGE("mprotect GOT failed for %s", symName);
                return h;
            }
            h.orig = (void*)*gotEntry;
            *gotEntry = (uintptr_t)replacement;
            __builtin___clear_cache((char*)gotEntry, (char*)gotEntry+8);
            mprotect((void*)pg, 0x1000, PROT_READ);
            h.ok = true;
            LOG("GOT patched for %s -> %p (was %p)", symName, replacement, h.orig);
            return h;
        }
    }
    LOGE("symbol %s not found in MC GOT", symName);
    return h;
}
...
static ssize_t hkRecv(int fd,void*buf,size_t l,int f,struct sockaddr*a,socklen_t*al){
    FnRecv orig=(FnRecv)gOrigRecv;
    ssize_t n=orig(fd,buf,l,f,a,al);
...
static float gDownX=0,gDownY=0,gDownRawX=0,gDownRawY=0,gBtnDragBX=0,gBtnDragBY=0;
...
static void processTouch(){
    ...
    if(t.down&&!gWasDown){
        ...
        if(fabsf(dx)>5||fabsf(dy)>5){
            gBtnOffX=gBtnDragBX-dx/gTMX*gSW;gBtnOffY=gBtnDragBY+dy/gTMY*gSH;
...
static void* initThread(void*){
    ...
    // Find MC base via dl_iterate_phdr
    PhdrCbData cbd={"libminecraftpe.so",0,0};
    dl_iterate_phdr(phdrCb,&cbd);
    gMcBase=cbd.base; gMcSize=cbd.size;
    LOG("MC base=0x%lx size=0x%lx",(unsigned long)gMcBase,(unsigned long)gMcSize);

    sleep(2);

    // Hook eglSwapBuffers via GlossHook (safe)
    ...
    if(pGlossHook&&swapSym){
        ...
        LOG("swap hook r=%d oSwap=%p",r,oSwap);
    }

    // Hook recvfrom/sendto via MC's GOT (safe — MC memory is patchable)
    if(gMcBase&&gMcSize){
        Hook2 rh=patchGOT(gMcBase,gMcSize,"recvfrom",(void*)hkRecv);
        if(rh.ok){gOrigRecv=rh.orig;LOG("recvfrom GOT hooked");}
        else LOG("recvfrom GOT not found (MC may use sendmsg instead)");

        Hook2 sh=patchGOT(gMcBase,gMcSize,"sendto",(void*)hkSend);
        if(sh.ok){gOrigSend=sh.orig;LOG("sendto GOT hooked");}
        else LOG("sendto GOT not found");
    }

    // Start touch thread
    pthread_t tt;pthread_create(&tt,nullptr,touchThread,nullptr);pthread_detach(tt);

    LOG("v11 ACTIVE swap=%p recv=%p send=%p",oSwap,gOrigRecv,gOrigSend);
    return nullptr;
}
...
static void onLoad(){
    LOG("LeviTracker v11 pid=%d",(int)getpid());
    const char*fp[]={
